...
require_once dirname( __DIR__ ) . '/OpenID/Store.php';
require_once dirname( __DIR__ ) . '/OpenID/HMAC.php';
...
class FileStore extends Store {
    ...
    /**
     * Initializes a new {@link FileStore}.  This
    ...
    {
        if (!\SimpleSAML\Module\openidprovider\Auth\Source\Auth\OpenID::ensureDir($directory)) {
    ...
    function destroy()
    {
        FileStore::_rmtree($this->directory);
        $this->active = false;
    }
    ...
    function _setup()
    {
        return (\SimpleSAML\Module\openidprovider\Auth\Source\Auth\OpenID::ensureDir($this->nonce_dir) &&
                \SimpleSAML\Module\openidprovider\Auth\Source\Auth\OpenID::ensureDir($this->association_dir) &&
                \SimpleSAML\Module\openidprovider\Auth\Source\Auth\OpenID::ensureDir($this->temp_dir));
    }
    ...
    function _mktemp()
    {
        $name = FileStore::_mkstemp($dir = $this->temp_dir);
        $file_obj = @fopen($name, 'wb');
        if ($file_obj !== false) {
            return [$file_obj, $name];
        } else {
            FileStore::_removeIfPresent($name);
        }
        return [];
    }
    ...
    function cleanupNonces()
    {
        ...
        $nonces = FileStore::_listdir($this->nonce_dir);
        $now = time();
        ...
        foreach ($nonces as $nonce_fname) {
            ...
            if (abs($timestamp - $now) > $Auth_OpenID_SKEW) {
                FileStore::_removeIfPresent($nonce_fname);
                $removed += 1;
            }
        }
        return $removed;
    ...
    function getAssociationFilename($server_url, $handle)
    {
        ...
        $parts = explode('/', $rest);
        $domain = FileStore::_filenameEscape($parts[0]);
        $url_hash = FileStore::_safe64($server_url);
        if ($handle) {
            $handle_hash = FileStore::_safe64($handle);
        } else {
            $handle_hash = '';
        }
    ...
    function storeAssociation($server_url, $association)
    {
        ...
        // around.
        FileStore::_removeIfPresent($tmp);
        return false;
    }
    ...
    function getAssociation($server_url, $handle = null)
    {
        ...
        if ($handle) {
            ...
            $association_files =
                FileStore::_listdir($this->association_dir);
            $matching_files = [];
    ...
    function _getAssociation($filename)
    {
        ...
        if (!$association) {
            FileStore::_removeIfPresent($filename);
            return null;
        }
    ...
    function removeAssociation($server_url, $handle)
    {
        ...
        if ($assoc === null) {
            ...
            $filename = $this->getAssociationFilename($server_url, $handle);
            return FileStore::_removeIfPresent($filename);
        }
    }
    ...
    function clean()
    {
        ...
        foreach ($this->_allAssocs() as $pair) {
            ...
            if ($assoc->getExpiresIn() == 0) {
                FileStore::_removeIfPresent($assoc_filename);
            }
        }
    }
    ...
    {
        $filename = "";
        $b = \SimpleSAML\Module\openidprovider\Auth\Source\Auth\OpenID::toBytes($str);

        for ($i = 0; $i < count($b); $i++) {
            $c = $b[$i];
            if (FileStore::_isFilenameSafe($c)) {
                $filename .= $c;
            } else {
